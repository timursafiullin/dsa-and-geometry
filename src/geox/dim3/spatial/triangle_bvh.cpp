...
namespace
{
    ...
    {
        geox::dim3::AABB bounds;
        geox::dim3::Point centroid;
    };

    BuildTriangle makeBuildTriangle(
        const geox::dim3::TriangleMesh& mesh,
        geox::dim3::TriangleId triangleId
    )
    {
        const geox::dim3::TriangleIndices& indices = mesh.triangle(triangleId);
        const geox::dim3::Point& a = mesh.vertex(indices[0]);
        const geox::dim3::Point& b = mesh.vertex(indices[1]);
        const geox::dim3::Point& c = mesh.vertex(indices[2]);
...
namespace geox::dim3
{
        ...
        while (!pending.empty())
        {
            ...
            if (node.triangleCount != 0)
            {
                for (std::uint32_t offset = 0; offset < node.triangleCount; ++offset)
                {
                    ...
                    const Point closest = closestPointOnTriangle(point, triangle);
    ...
    )
    {
        ...
        ) -> TriangleBVH::NodeId
        {
            ...
            AABB triangleBounds = buildTriangles.at(
            ...
            for (std::uint32_t offset = 1; offset < triangleCount; ++offset)
            {
                ...
                centroidBounds = merge(
                    centroidBounds,
                    AABB{triangle.centroid, triangle.centroid}
                );
            }
