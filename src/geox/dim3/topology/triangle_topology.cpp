...
namespace
{
    ...
    std::size_t findLocalEdge(
        const geox::dim3::TriangleTopology::TriangleEdges& triangleEdges,
        geox::dim3::EdgeId edgeId
    )
    {
...
namespace geox::dim3
{
    ...
    TriangleTopology buildTriangleTopology(const TriangleMesh& mesh)
    {
        ...
        return detail::buildTriangleTopology(mesh.vertexCount(), mesh.triangles());
    }

    TriangleTopology detail::buildTriangleTopology(
    ...
    )
    {
        ...
        topology.vertexCount_ = vertexCount;
        topology.triangleCount_ = triangles.size();
        ...
        for (TriangleId triangleId = 0; triangleId < triangles.size(); ++triangleId)
        {
            VertexId v0 = triangles.at(triangleId).at(0);
            VertexId v1 = triangles.at(triangleId).at(1);
            VertexId v2 = triangles.at(triangleId).at(2);
