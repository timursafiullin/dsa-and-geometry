...
namespace
{
    ...
    void validateMeshAndTopology(
        ...
        const geox::dim3::TriangleTopology& topology
    )
    {
        if (!mesh.isValid())
    ...
    bool hasTriangle(
        const geox::dim3::EdgeIncidence& incidence,
...
namespace geox::dim3
{
    ...
    double signedVolume(const TriangleMesh& mesh)
    {
        ...
        {
            const Point& a = mesh.vertex(indices[0]);
            const Point& b = mesh.vertex(indices[1]);
            const Point& c = mesh.vertex(indices[2]);
