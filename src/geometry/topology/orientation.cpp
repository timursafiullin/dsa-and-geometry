...
namespace
{
    ...
    void validateMeshAndTopology(
        const dsa::geometry::TriangleMesh& mesh,
        const dsa::geometry::TriangleTopology& topology
    )
    {
        if (!mesh.isValid())
    ...
    bool hasTriangle(
        const dsa::geometry::EdgeIncidence& incidence,
        dsa::geometry::TriangleId triangleId
    )
    {
        return std::find(
